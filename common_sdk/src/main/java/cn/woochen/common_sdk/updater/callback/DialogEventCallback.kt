...
interface DialogEventCallback {
     ...
     * 在用户做出相应的操作后，你应当调用[Updater.setCheckHandlerResult]方法进行下一步的操作。
     *
     * @param updater [Updater]对象。
     * @param isForce 是否是强制更新。
     * @see Updater.setCheckHandlerResult
     */
    fun onShowCheckHintDialog(updater: Updater, updateInfo: UpdateInfo, isForce: Boolean)

    /**
     * 当需要加载下载进度对话框的时候调用，你需要在这做显示下载进度对话框的操作。
     *
     * 也有可能这个方法会调用不止一次，如果你在构建 [Updater.Builder] 的时候没有调用
     * [Updater.Builder.setCheckWiFiState]方法改变检测网络状态的话默认是会检测WIFI状态的。当WIFI状态改变的时候
     ...
     *
     * @param isForce 是否是强制更新。
     * @see Updater.Builder.setCheckWiFiState
     * @see .onProgress
     */
    fun onShowProgressDialog(isForce: Boolean)
